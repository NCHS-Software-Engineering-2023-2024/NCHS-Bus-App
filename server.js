...
connection2.connect((err) => {
  ...
  //SQL query to select all rows from the table
  const sqlQuery = 'SELECT * FROM Whitelist';

  //Execute query
  connection2.query(sqlQuery,(queryErr,results) =>{
    if(queryErr){
      console.error('Error executing query:',queryErr);
      connection2.end(); //close connection
      return;
    }
...
app.post("/updateStatus", (req, res) => {
  ...
  fs.readFile("buslist.json", "utf-8", (err, jsonString) => {
    ...
    connection.connect((err) => {
      ...
      //SQL query to select all rows from the table
      const sqlQuery = 'SELECT * FROM Subscriptions WHERE busNum = ' + bus.number;
      ...
      connection.query(sqlQuery,(queryErr,results) =>{
        ...
        for (const row of results) {
          ...
          webpush.sendNotification(JSON.parse(row.subscription), payload).catch(err => console.error(err));
...
const publicVapidKey = 'BKwZwHIXofitiwaHEbYarw-dVvCOtTdJqKRuxPhGa6EqPsWx1DRnQiW79ZGCSj7-NhP5Rv-eio_beOuDTwJrYf0';
const privateVapidKey = 'X91X3FZ4ZVAIMAJEYbZs7S8Pl8-RyOBb1Eim2uTq134';
...
app.post('/subscribe', (req, res) => {
  ...
  connection.connect((err) => {
    ...
    //SQL query to select all rows from the table
    const sqlQuery = 'INSERT INTO Subscriptions (busNum, subscription) VALUES(?,?)';
    let values = [3,JSON.stringify(subscription)];
